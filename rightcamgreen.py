...
def node1cb(data, args ):
    ...
    maskmain = cv2.addWeighted(maskgreen,1.0,maskred,0,0)
...
def node1():
    ...
    cv2.createTrackbar('gHMax','filter',161,255,nothing)
    ...
    cv2.createTrackbar('rSMin','filter',101,255,nothing)
    cv2.createTrackbar('rVMin','filter',124,255,nothing)

    cv2.createTrackbar('rHMax','filter',49,255,nothing)
    cv2.createTrackbar('rSMax','filter',255,255,nothing)
